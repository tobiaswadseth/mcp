...
const ClassDataStore = () => {
  this.store = {};
};

ClassDataStore.prototype.loadFile = (fileName, external) => {
  return new Promise((resolve, reject) => {
    fs.readFile(
      external
        ? path.join(app.getPath("userdata"), "jjdoc", fileName + ".json")
        : path.join(__dirname, "../data/classes", fileName + ".json"),
      "utf-8",
      (err, data) => {
        if (err) {
          reject();
...
ClassDataStore.prototype.init = () => {
  return Promise.all([this.loadFile("java"), this.loadFile("spigot")]);
};

ClassDataStore.prototype.size = () => {
  return Object.keys(this.store).length;
};

ClassDataStore.prototype.getClassesByName = () => {
  return this.store;
};

ClassDataStore.prototype.getClass = (className) => {
  if (!className) return null;
  return this.store[className.toLowerCase()];
};

ClassDataStore.prototype.getField = (className, fieldName) => {
...
ClassDataStore.prototype.getConstructor = (className, constructorSignature) => {
...
ClassDataStore.prototype.getConstructorParam = (
  className,
...
ClassDataStore.prototype.getMethod = (className, methodSignature) => {
...
ClassDataStore.prototype.getMethodParam = (
  className,
  methodSignature,
  paramName
) => {
...
ClassDataStore.prototype.getMethodSignatureFromMethodAndParamTypes = (
  methodName,
  paramTypes
) => {
...
ClassDataStore.prototype.getMethodSignatureFromData = (methodData) => {
  let params = [];
...
ClassDataStore.prototype.getAllImplementingClasses = (rootClass) => {
  return getImplementingClasses(this.store, rootClass, []);
};

ClassDataStore.prototype.getAllExtendingClasses = (rootClass) => {
  return getExtendingClasses(this.store, rootClass, []);
};

ClassDataStore.prototype.getAllImplementingAndExtendingClasses = (
  rootClass
) => {
  return getImplementingAndExtendingClasses(this.store, rootClass, []);
};

const getImplementingAndExtendingClasses = (store, className, target) => {
...
const getImplementingClasses = (store, className, target) => {
  let clazz = store[className.toLowerCase()];
  if (target.indexOf(className) === -1) target.push(className);
...
const getExtendingClasses = (store, className, target) => {
  let clazz = store[className.toLowerCase()];
  if (target.indexOf(className) === -1) target.push(className);
